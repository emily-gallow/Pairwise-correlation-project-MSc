"""Behavioural mismatch against cross-day NOISE RSA, adjusted for the actual
inter-session interval. Companion to script 41.

    noise_RSA_ij = b0 + b1 * mismatch_ij + b2 * actual_gap_ij + u_i + e_ij
...
Outputs:
  <root>/noise_rsa_trial_mismatch_actual_stats.csv
  <root>/noise_rsa_trial_mismatch_actual.png
"""
import argparse
...
root = Path(args.root)
df = load_with_actual_gaps(root / "noise_rsa_trial_mismatch.csv",
...
pd.DataFrame(results).to_csv(
    root / "noise_rsa_trial_mismatch_actual_stats.csv", index=False)
...
for ax, (proxy_col, proxy_label, panel_title) in zip(axes, PROXIES):
    ...
    ax.set_ylabel("Cross-day noise RSA (Pearson r)", fontsize=12)
    ax.set_title(panel_title, fontsize=13)
    ax.grid(alpha=0.3)
    ax.legend(loc="upper right", fontsize=9, frameon=True)

fig.suptitle("Cross-day noise RSA vs behavioural mismatch", fontsize=13, y=1.02)
plt.tight_layout()
out = root / "noise_rsa_trial_mismatch_actual.png"
